...
func differenceInVolumes(cube1: [Int], cube2: [Int]) -> Int? {
    ...
    guard cube1.count == 3, cube2.count == 3 else {
        return nil
    }
    
    let volume1 = cube1.reduce(1, *)
    let volume2 = cube2.reduce(1, *)
    
    return abs(volume1 - volume2)
}

let result = differenceInVolumes(cube1: [2, 2, 3], cube2: [5, 4, 1])
print(result!)

// Force unwrapped to get rid of warning
