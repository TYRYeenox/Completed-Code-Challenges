...
func currencyFormatting(money: Float) -> String {
    let dollarSign = NumberFormatter()
    dollarSign.locale = Locale(identifier: "en_US")
    dollarSign.numberStyle = .currency
    return dollarSign.string(from: NSNumber(value: money)) ?? ""
}

let money = currencyFormatting(money: 39.99)
print(money)
