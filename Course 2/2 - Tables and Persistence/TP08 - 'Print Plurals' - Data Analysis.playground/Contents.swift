...
func printPluralStrings(_ words: [String]) {
    for word in words {
        if word.count > 0 {
            let lastLetter = word[word.index(before: word.endIndex)]
            if lastLetter == "s" {
                print(word)
            }
        }
...
let words = ["Apple", "Oranges", "Pineapple", "Bananas", "Grapes", "Raspberry"]
printPluralStrings(words)
