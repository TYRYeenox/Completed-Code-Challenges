...
func wordFrequency(in text: String) -> [String: Int] {
    ...
    for word in words {
        ...
        if let count = frequencies[wordString] {
            frequencies[wordString] = count + 1
        } else {
            frequencies[wordString] = 1
        }
    }
...
let sentence = "bloop bloop bleep"
let result = wordFrequency(in: sentence)
print(result)
