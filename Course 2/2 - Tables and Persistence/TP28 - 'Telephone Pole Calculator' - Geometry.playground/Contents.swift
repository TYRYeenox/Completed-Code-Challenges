...
func distanceBetweenPoles(numOfPillars: Int, distance: Int, radius: Int) -> Int {
    if numOfPillars <= 1 {
        return 0
    }
    
    let gaps = numOfPillars - 1
    
    let distanceInCentimeters = distance * 100
    
    let totalDistanceBetweenPoles = gaps * distanceInCentimeters
    
    let totalPoleWidth = gaps * (2 * radius)
    
    return totalDistanceBetweenPoles + totalPoleWidth
}


let result = distanceBetweenPoles(numOfPillars: 5, distance: 25, radius: 10)
print(result)
